...
export function ProblemSolvingWorkflow() {
  ...
  async function importEvent() {
    ...
    setEvent(next); setAnalysis(null); setSaved(null); setProblemStatement(next.description); setChosen([]); setSteps([]); setNotes(""); setHistory([]); setError(""); setMessage("");
    ...
    if (data.storage === "device") {
      try {
        ...
        if (local) {
          ...
          setAnalysis(restoredAnalysis); setSaved(progress.saved); setProblemStatement(progress.problemStatement || next.description); setChosen(progress.selectedSolutionIds); setSteps(progress.nextSteps); setHistory(progress.history);
  ...
  const updateCauseFindings = (category: CauseCategory["category"], findings: string[]) => setAnalysis((current) => current ? { ...current, causes: current.causes.map((cause) => cause.category === category ? { ...cause, findings } : cause) } : current);
